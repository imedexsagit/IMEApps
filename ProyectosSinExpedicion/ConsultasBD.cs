    ...
    class ConsultasBD
    {
        ...
        public void recuperarFichajesSinExpedicion (string proyecto)
        {
            ...
            try
            {
                ...
                SqlCommand sqlCmd = new SqlCommand("Ime_recuperarFichajesProyectosSinExpedicion", conexion);
                sqlCmd.CommandType = CommandType.StoredProcedure;


                sqlCmd.Parameters.Add("@PROYECTO", SqlDbType.VarChar);
                sqlCmd.Parameters["@PROYECTO"].Value = proyecto;
        ...
        public DataTable ObtenerInformacionDiaria(string fechaMin, string fechaMax)
        {
            ...
            try
            {
                ...
                strsql = @"Select FechaFiltro as Fecha,SumNegro,SumBlanco, Diferencia as 'Kg Zn arrastre' ,((Diferencia/SumNegro) * 100)  as 'Porcentaje arrastre', (PromedioMin/TotalFilas) as PromMin, (PromedioMed/TotalFilas) as PromMed, (PromedioMax/TotalFilas) as PromMax,TotalFilas as 'Total Perchas Galvanizadas'  from (
...
SELECT   T_ORDFAB.CODIGO, T_ORDFAB.ORDFAB, T_ORDFAB.CTDLAN,
				        ...
				        CASE WHEN dbo.IME_Tiene_OF_Ope(T_ORDFAB.CODEMP, T_ORDFAB.ORDFAB, dbo.IME_GetOpPlegadoraCH()) = 1 THEN isnull(SUM(Vista_TC_IMPORTBONOS.PlegCH), 0) ELSE NULL END AS PlegCH, 
                         ...
                         CASE WHEN dbo.IME_Tiene_OF_Ope(T_ORDFAB.CODEMP, T_ORDFAB.ORDFAB, dbo.IME_GetOpContornear()) = 1 THEN isnull(SUM(Vista_TC_IMPORTBONOS.Contor), 0) ELSE NULL END AS P2, 
                         CASE WHEN dbo.IME_Tiene_OF_Ope(T_ORDFAB.CODEMP, T_ORDFAB.ORDFAB, dbo.IME_GetOpAvellanar()) = 1 THEN isnull(SUM(Vista_TC_IMPORTBONOS.Avella), 0) ELSE NULL END AS P3, 
...
select tab1.Espesor, PesoRealMarcaAgrupada as 'PesoKg' , Toneladas,Porcentaje as Relacion, Consumo from ( 
                            ...
                            when EspesorPredominante >= 21 and EspesorPredominante<=25 then '(21-25)'
                            when EspesorPredominante >= 26 and EspesorPredominante<=30 then '(26-30)'
                            when EspesorPredominante >= 31 and EspesorPredominante<=35 then '(21-35)' else 'Otros'
                            ...
                            when EspesorPredominante >= 21 and EspesorPredominante<=25 then '(21-25)'
                            when EspesorPredominante >= 26 and EspesorPredominante<=30 then '(26-30)'
                            when EspesorPredominante >= 31 and EspesorPredominante<=35 then '(21-35)'
                            else 'Otros' end as Espesor, PesoPercha, Utiles, cast (PesoNegro as float) as PesoNegro, PesoNegroSinPercha, PesoBlanco, cast (DiferenciaZinc as float) DiferenciaZinc, Porcentaje, cast(PEsoRealMarca as DECIMAL(18, 2)) as PesoRealMarca, EspesorPredominante, 
