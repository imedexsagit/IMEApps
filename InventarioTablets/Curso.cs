...
namespace InventarioTablets
{
    public partial class Curso : Form
    {
        ...
        public Curso(String emp, Int32 anio, String cad, String disp, /*Int32 min, Int32 max,*/ String mat, ArrayList lista_maq, String lugar)
        {
            InitializeComponent();
            ...
            for (int x = 0; x < lista_maq.Count; x++)
            {
                this.comboBox1.Items.Add(lista_maq[x]);
            }
        ...
        //en cuanto se elige la máquina paso el control a la etiqueta
        ...
        private Boolean existeEtiqueta(String etiq)
        {
            try
            {
                this.conn.Open();
                ...
                strSql += "         FROM TC_LANZA_ETIQUETA";
                strSql += "         WHERE (CodEmp = '" + this.CodEmp + "') AND (Etiqueta = " + etiq + ")";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if ((textBox1.Text == String.Empty) || (textBox2.Text == String.Empty) || (textBox3.Text == String.Empty) || (textBox4.Text == String.Empty) || (comboBox1.SelectedItem.ToString() == String.Empty))
            {
            ...
            {
                if (existeEtiqueta(textBox3.Text) == false)
                {
                    MessageBox.Show("La etiqueta no existe, hable con el Dpto de Produccion", "Error", MessageBoxButtons.OK, MessageBoxIcon.Exclamation, MessageBoxDefaultButton.Button1);
            ...
            try
            {
                ...
                #region obtengo el articulo y el proyecto asociado a la etiqueta
                String strSql = "SELECT T_ORDFAB.CODIGO, T_ORDFAB.proyecto FROM TC_LANZA_ETIQUETA INNER JOIN T_ORDFAB ON TC_LANZA_ETIQUETA.CodEmp = T_ORDFAB.CODEMP AND TC_LANZA_ETIQUETA.TIPOREG = T_ORDFAB.TIPOREG AND TC_LANZA_ETIQUETA.OrdFab = T_ORDFAB.ORDFAB";
                strSql += "         WHERE (TC_LANZA_ETIQUETA.codemp = '" + this.CodEmp + "') AND (TC_LANZA_ETIQUETA.Etiqueta=" + this.textBox3.Text + ")";
                ...
                while (reader.Read())
                {
                    marca = reader.GetValue(0).ToString();
                    proyecto = Convert.ToInt32(reader.GetValue(1));
                }
                ...
                #region compruebo que la etiqueta pase x la maquina indicada
                String aux_maquina = this.comboBox1.SelectedItem.ToString();
                aux_maquina = aux_maquina.Substring(0, aux_maquina.IndexOf("-->"));


                //JR 28/11/2011
                strSql = " ";
                strSql = "SELECT COUNT(T_ORDFABO.PUESTO)";
                strSql += "	FROM TC_LANZA_ETIQUETA INNER JOIN T_ORDFABO ON TC_LANZA_ETIQUETA.CodEmp = T_ORDFABO.CODEMP AND TC_LANZA_ETIQUETA.TIPOREG = T_ORDFABO.TIPOREG AND TC_LANZA_ETIQUETA.OrdFab = T_ORDFABO.ORDFAB";
                strSql += "	WHERE (TC_LANZA_ETIQUETA.CodEmp = '" + this.CodEmp + "') AND (TC_LANZA_ETIQUETA.TIPOREG = 'F') AND (TC_LANZA_ETIQUETA.Etiqueta = " + this.textBox3.Text + ") AND (T_ORDFABO.operac = gg.dbo.IME_GetOpPuesto('" + this.CodEmp + "', '" + aux_maquina + "'))";


                System.Data.SqlClient.SqlCommand comando3 = new SqlCommand(strSql, this.conn);

                Int32 auxi = Convert.ToInt32(comando3.ExecuteScalar());
                comando3.Dispose();

                if (auxi == 0)
                {
                    MessageBox.Show("La máquina donde indica que se encuentra esa etiqueta no es correcta.", "Error: Máquina incorrecta", MessageBoxButtons.OK, MessageBoxIcon.Hand, MessageBoxDefaultButton.Button1);
                ...
                strSql = " ";
                strSql = "INSERT INTO TC_inventario_contado (CodEmp, Codigo, Categoria, Familia, Cantidad, Ficha, Taco, Ano, Tipo, Proyecto, maquina, Observaciones, FechaC, usuarioC, centro)";
                strSql += " SELECT CodEmp, Codigo, Categoria, Familia," + this.textBox4.Text + ", " + this.textBox2.Text + ", " + this.textBox1.Text + ", " + this.AnioInventario.ToString() + ", '" + this.TipoMaterial + "', " + proyecto.ToString() + ", " + aux_maquina + ", " + this.textBox3.Text + ", getdate(), '" + this.PDA + "', '" + this.Lugar + "'";
                strSql += "     FROM T_Articulos WHERE codemp = '" + this.CodEmp + "' AND codigo = '" + marca + "'";
        ...
        private void Curso_Closed(object sender, EventArgs e)
        {
            if (conn != null)
        ...
        private void btnBorrarLote_Click_1(object sender, EventArgs e)
        {
            this.textBox3.Clear();
        ...
        private void btnHistorial_Click(object sender, EventArgs e)
        {
            ...
            string strsql = "";
            strsql = strsql + " select TACO, Ficha as FICHA, Codigo AS CODIGO, Tipo AS TIPO, fechaC AS FECHA from TC_Inventario_contado where Tipo = 'Curso' and Ano = '" + AnioInventario + "' and Centro like '" + Lugar + "' and usuarioC = '" + this.PDA + "' ";
